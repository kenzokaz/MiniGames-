...
public class TicTacToe extends Application {
	...
	public void start(Stage primaryStage) {
		TextInputDialog dialogX = new TextInputDialog("Player A");
		dialogX.setTitle("Player A Name");
		dialogX.setHeaderText(null);
		dialogX.setContentText("Enter Player A's Name:");
		Optional<String> resultX = dialogX.showAndWait();
		resultX.ifPresent(name -> playerXName = name);

		TextInputDialog dialogO = new TextInputDialog("Player B");
		dialogO.setTitle("Player B Name");
		dialogO.setHeaderText(null);
		dialogO.setContentText("Enter Player B's Name:");
	...
	private Button createCell() {
		...
		cell.setStyle(
				"-fx-font-size: 2em; -fx-background-color: white; -fx-text-fill: black; -fx-border-color: black;");
		cell.setOnAction(event -> {
			if (cell.getText().isEmpty()) {
				cell.setText(String.valueOf(currentPlayer));
				if (currentPlayer == 'X') {
					cell.setStyle(
							"-fx-font-size: 2em; -fx-background-color: white; -fx-text-fill: red; -fx-border-color: black;");
				} else {
					cell.setStyle(
							"-fx-font-size: 2em; -fx-background-color: white; -fx-text-fill: blue; -fx-border-color: black;");
				}
				if (checkForWin()) {
					showWinDialog();
					resetBoard();
